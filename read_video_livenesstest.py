...
def main():
    ...
    # FPS計測モジュール ########################################################
    cvFpsCalc = CvFpsCalc(buffer_len=1000)
    path = ("/home/doanmsi/Music/Doanhm-liveness-test/videos")
    for path_vd in os.listdir(path):
        path_absolute = str(path) + "/" + str(path_vd) 
        # print(path_absolute)
        # video_path = ('/home/doanmsi/Music/Doanhm-liveness-test/videos/672.mp4')
        cap = cv.VideoCapture(path_absolute)
        count_liveness = 0
        count_spoof = 0
        while True:
            ...
            max_index = np.argmax(as_result)
            if max_index ==0:
                count_liveness = count_liveness + 1
            if max_index ==1:
        ...
        print("So anh mat that trong video: " + str(count_liveness))
        print("So anh mat gia trong video: " + str(count_spoof))
        f = open("label.txt", "a")
        if count_liveness > count_spoof:
            print(str(path_vd)+ ",0")
            f.write(str(path_vd)+ ",0")
            f.write("\n")
        else:
            print(str(path_vd)+ ",1")
            f.write(str(path_vd)+ ",1")
            f.write("\n")
        cap.release()
...
def draw_detection(
    ...
    for bbox, as_result in zip(bboxes, as_results):
        ...
        if as_index == 0:
            cv.putText(image, ('Liveness: ' + str(as_index)),
                       (bbox[0], bbox[1] - 10), cv.FONT_HERSHEY_SIMPLEX, 0.7,
                       (255, 0, 0), 1, cv.LINE_AA)
            
        elif as_index == 1:
            cv.putText(image, 'Spoof: ' + str(as_index), 
                       (bbox[0], bbox[1] - 10), cv.FONT_HERSHEY_SIMPLEX, 0.7,
